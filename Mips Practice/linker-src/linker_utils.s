...
relocate_inst:
	...
	# YOUR CODE HERE
	add $s0, $0, $a0		#save the registers
	add $s1, $0, $a1
	
	add $a0, $0, $a3		#prepare for function call
	add $a1, $a1, $0		#prepare for function call
	jal symbol_for_addr		#search for ths symbol for the addr
	beq $v0, $0, relocerror		
	
	add $a0, $a2, $0		
	add $a1, $v0, $0
	jal addr_for_symbol		#search for absolute addr of the symbol
	
	slt $t0, $v0, $0		#if less than 0 is reloc error
	bne $t0, $0, relocerror
	
	srl $s0, $s0, 26		# map 0-25 bit to 0
	sll $s0, $s0, 26		# map 0-25 bit to 0
	srl $v0, $v0, 2
	add $v0, $v0, $s0
	j relocexit
relocerror:
