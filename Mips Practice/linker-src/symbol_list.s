...
addr_for_symbol:
	...
	#store add of list to $s0 and $s1
	add $s0, $a0, $0
	add $s1, $a1, $0
	beq $s0, $0, afsexit
	
afsloop:
	#address of string to $2
	addiu $s2, $s0, 4
	lw $s2, 0($s2)
	add $t0, $0, $s2 #t0 is symbol addr
	add $t1, $0, $s1 #t1 is target addr
	#compare the string
strcmploop:
	lb $t2, 0($t0)
	lb $t3, 0($t1)
	beq $t2, $0, strcmpfirstequal
	beq $t3, $0, strcmpnotequal
	bne $t2, $t3, strcmpnotequal
	addiu $t0, $t0, 1
	addiu $t1, $t1, 1
	j strcmploop
strcmpfirstequal:
	beq $t3, $0, strcmpequal
strcmpnotequal:
	addiu $s2, $s0, 8
	lw $s0, 0($s2) #point to the next address stored in s0
	beq $s0, $0, afsexit #null next ptr, exit with -1
	j afsloop
afsexit:
	addiu $v0, $0, -1
afsspmanage:
